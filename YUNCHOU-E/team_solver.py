...
def route_to_trip(route, data, cfg, corridors):
    ...
    depot = next((n for n in data.depots if n.name == route.depot_name), data.depots[0])
    current = depot
    segments = []
    order_ids = []
    weight = 0.0
    ...
    return {
        ...
        "orders": order_ids,
        "segments": segments,
        "time_s": float(route.time) * 3600.0,
        "energy_wh": 0.0,
        "late_s": float(route.late) * 3600.0,
        "weight": weight,
    }
...
def solution_to_result(sol, data, cfg, method, elapsed):
    ...
    result = {
        ...
        "summary": {
            ...
            "total_time_h": float(sol.total_time),
            "total_time_s": float(sol.total_time) * 3600.0,
            "total_energy_wh": 0.0,
