...
def eval_solution(assignment, data, cfg):
    ...
    if len(assignment) != m or any(d <= 0 or d > n_drones for d in assignment):
        return 1e9, 0, 0, False
    if any(order.weight > cfg.W_max for order in data.orders):
        return 1e9, 0, 0, False
    ...
    for i in range(n_drones):
        ...
        while unassigned_queue:
            ...
            if e_ret > cfg.E_max:
                return 1e9, 0, 0, False
            if trip_energy + e_ret > cfg.E_max:
                total_energy += trip_energy
                total_late += trip_late
                total_swaps += 1
                drone_time += cfg.t_swap_min / 60
